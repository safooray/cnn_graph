...
class base_model(object):
	...
	def predict(self, data, labels=None, sess=None):
		...
		for begin in range(0, size, self.batch_size):
			...
			predictions[begin:end] = batch_pred[:end-begin].squeeze()

		if labels is not None:
			return predictions, loss * self.batch_size / size
		else:
			return predictions
	def cox_evaluate(self, data, labels, observed, at_risk, sess=None):
		"""
		...
		labels: size N
			N: number of signals (samples)
		"""
		t_process, t_wall = time.time(), time.time()
		pred_val = self.predict(data, None, sess)
		#print(predictions)
		#feed_dict = {self.ph_data: data, self.ph_dropout: self.dropout}
		#pred_val = sess.run([self.op_logits], feed_dict)
		cindex_val = self.sa.c_index(pred_val, labels, 1-observed)
		string = 'accuracy: {:.2f}'.format(cindex_val)
		if sess is None:
			string += '\ntime: {:.0f}s (wall {:.0f}s)'.format(time.time()-t_process, time.time()-t_wall)
		return string, cindex_val

	def cox_fit(self, train_data, train_labels, train_observed, val_data, val_labels, val_observed):
		t_process, t_wall = time.time(), time.time()
		...
		for step in range(1, num_steps+1):
			...
			batch_data, batch_labels, batch_observed = train_data[idx,...], train_labels[idx], train_observed[idx]
			batch_data, batch_labels, batch_observed, batch_atRisk = self.sa.calc_at_risk(batch_data, batch_labels, batch_observed) 
			if type(batch_data) is not np.ndarray:
				batch_data = batch_data.toarray()  # convert sparse matrices
			feed_dict = {self.ph_data: batch_data, self.ph_labels: batch_labels, self.ph_observed:batch_observed, self.ph_atRisk:batch_atRisk, self.ph_dropout: self.dropout}
			learning_rate, loss_average, pred = sess.run([self.op_train, self.op_loss_average, self.op_logits], feed_dict)
		#	learning_rate, loss_average, pred = sess.run([self.op_train, self.op_loss, self.op_logits], feed_dict)
			# Periodical evaluation of the model.
			cindex = self.sa.c_index(pred, batch_labels, 1-batch_observed)
			val_data, val_labels, val_observed, val_atRisk = self.sa.calc_at_risk(val_data, val_labels, val_observed) 
			if step % self.eval_frequency == 0 or step == num_steps:
				epoch = step * self.batch_size / train_data.shape[0]
				print('step {} / {} (epoch {:.2f} / {}):'.format(step, num_steps, epoch, self.num_epochs))
				print('  learning_rate = {:.2e}, loss_average = {:.2e}, cindex = {:06.2f}'.format(learning_rate, loss_average, cindex))
				string, cindex_val = self.cox_evaluate(val_data, val_labels, val_observed, val_atRisk, sess)
				cindices_val.append(cindex_val)
				print('  validation {}'.format(string))
				#print('  time: {:.0f}s (wall {:.0f}s)'.format(time.process_time()-t_process, time.time()-t_wall))
				losses.append(loss_average)
				# Save model parameters (for evaluation).
				self.op_saver.save(sess, path, global_step=step)
				np.save('acc_results/'+ self.description+'_ci_val.npy', cindices_val)
				np.save('acc_results/'+ self.description+'loss_trn.npy', losses)

		print('validation accuracy: peak = {:.2f}, mean = {:.2f}'.format(max(cindices_val), np.mean(cindices_val[-10:])))
		sess.close()

		t_step = (time.time() - t_wall) / num_steps
		return cindices_val, losses, t_step
	...
	def build_graph(self, M_0):
		...
		with self.graph.as_default():
			...
			with tf.name_scope('inputs'):
				...
				self.ph_observed = tf.placeholder(tf.float32, (self.batch_size), 'observed')
				self.ph_atRisk = tf.placeholder(tf.int32, (self.batch_size), 'atRisk')
				self.ph_dropout = tf.placeholder(tf.float32, (), 'dropout')

			# Model.
			self.op_logits = self.inference(self.ph_data, self.ph_dropout)
			self.op_loss, self.op_loss_average = self.cox_pll_loss(self.op_logits, self.ph_labels, self.ph_observed, self.ph_atRisk, self.regularization)
			self.op_train = self.training(self.op_loss, self.learning_rate,
					self.decay_steps, self.decay_rate, self.momentum)
			self.op_prediction = self.risk_prediction(self.op_logits)
	...
	def cox_pll_loss(self, logits, labels, observed, at_risk, regularization):
		"""Adds to the inference model the layers required to generate loss."""
		with tf.name_scope('loss'):
			with tf.name_scope('cox_pll'):
				#Cox loss
				labels = tf.to_int64(labels)
				fac_logits = logits - tf.reduce_max(logits, axis=0)
				exp = tf.exp(fac_logits)
				partial_sum = tf.cumsum(exp, reverse=True) + 1
				log_at_risk = tf.log(tf.gather(partial_sum, tf.reshape(at_risk, [-1]))) + tf.reduce_max(logits, axis=0)
				print("HERE: ", partial_sum.get_shape(), log_at_risk.get_shape())
				diff = tf.sub(logits,log_at_risk)
			...
			with tf.name_scope('averages'):
				...
				op_averages = averages.apply([cost, regularization, loss])
	...
	def risk_prediction(self, logits):
		"""Return the predicted classes."""
		with tf.name_scope('prediction'):
			prediction = logits
			return prediction
...
class cgcnn(base_model):
	...
	def __init__(self, L, F, K, p, M, filter='chebyshev5', brelu='b1relu', pool='mpool1',
		...
		self.description = str(len(F))+'layers-'
		for f in F: self.description += str(f)+'-'
		self.description += 'pooling-'
		for pp in p: self.description += str(pp)+'-'
		self.description += str(regularization)
		self.description += '-'
		self.description += str(dropout)
		...
		for i in range(len(self.p)):
			with tf.variable_scope('conv{}'.format(i+1)):
				with tf.name_scope('filter'):
					print (self.L[i].shape, self.F[i], self.K[i])
